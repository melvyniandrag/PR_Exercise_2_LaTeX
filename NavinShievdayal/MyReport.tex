...
\iknow{cp -r}{copies to directories}
\iknow{mv}{renames files}
\iknow{rm}{removes/deletes files}
\iknow{rm -r}{removes/deletes directories}
\iknow{ls -a}{lists all files, even those hidden}
\iknow{ls -al}{list files, and perms on drop down}
\iknow{ls -l}{list unhidden files}
\iknow{ssh}{connector to other device}
\iknow{ssh-keygen}{creates key}
\iknow{cd}{moves into or changes directory}
\iknow{cd ../}{goes back directories}
\iknow{cd \textasciitilde}{home directory}
\iknow{echo}{calls out or regular expression in file}
\iknow{cat{concatenates file}
\iknow{adduser}{add local user}
\iknow{deluser}{delete local user}
\iknow{chmod}{changes perms of file}
\iknow{chown}{changes owner of file}
