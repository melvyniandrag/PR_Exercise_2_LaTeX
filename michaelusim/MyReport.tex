...
\iknow{cp -r}{To copy recursivelyo}
\iknow{mv}{this is used to move fileso}
\iknow{rm}{this is used to delete fileso}
\iknow{rm -r}{this is used to delete recursively}
\iknow{ls -a}{list all files including the hidden fileso}
\iknow{ls -la}{List with long format and show hidden fileso}
\iknow{ls -l}{List with long format}
\iknow{ssh}{Connect to system using ssh protocol }
\iknow{ssh-keygen}{create ssh key pairs}
\iknow{cd}{used to traverse linux file directorieso}
\iknow{cd ../}{go back two directories}
\iknow{cd \textasciitilde}{used to go to home directory}
\iknow{echo}{print values passed as arguments }
\iknow{cat}{reads content of files and writes to standard output}
\iknow{adduser}{used to add user}
\iknow{deluser}{used to delete user}
\iknow{chmod}{used to change permissions}
\iknow{chown}{used to change owner of a file or directory }
